import pytest
from .throttling import ThrottledBaseClient, MaxRequestSizeExceeded, MaxRequestsExceeded


def test_size_throttling_on_small_requests():
...
def test_size_throttle_on_large_request():
    cli = ThrottledBaseClient()
...
def test_low_threshold_requests_does_not_raise():
    cli = ThrottledBaseClient()
    for _ in range(4):
        cli.throttle_max_requests()


def test_max_requests_raises_exception():
    cli = ThrottledBaseClient()
